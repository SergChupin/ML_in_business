...
def general():
	return """Welcome to stroke prediction process. Please use 'http://<address>/predict' to POST"""
...
def predict():
	...
	if flask.request.method == "POST":
		...
		if request_json["hypertension"]:
			company_profile = request_json['hypertension']

		if request_json["heart_disease"]:
			benefits = request_json['heart_disease']

		if request_json["smoking_status"]:
			benefits = request_json['smoking_status']
		logger.info(f'{dt} Data: age={age}, '
					f'hypertension={hypertension}, '
					f'heart_disease={heart_disease}'
